...
| [`update_collection`](./operations/update_collection.md) | Patch `hnsw_config`, `quantization_config`, `optimizers_config`, payload storage, or per-vector params in place | ✘ (prior state unknown) |
...
| [`create_payload_index`](./operations/create_payload_index.md) | Index a payload field, with optional index `params` | ✔ → `delete_payload_index` |
